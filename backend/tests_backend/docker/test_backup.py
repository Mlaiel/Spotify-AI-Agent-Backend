...
def test_backup_script_exists():
    assert os.path.isfile(BACKUP_SH), f"Fichier introuvable: {BACKUP_SH}"
...
def test_backup_script_runs():
    result = subprocess.run([BACKUP_SH, '--dry-run'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=15)
