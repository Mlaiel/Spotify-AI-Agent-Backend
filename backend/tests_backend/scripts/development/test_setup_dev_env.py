...
def test_setup_dev_env_exists():
    assert os.path.isfile(SCRIPT), f"Fichier introuvable: {SCRIPT}"

def test_setup_dev_env_permissions():
    st = os.stat(SCRIPT)
    assert stat.S_IMODE(st.st_mode) & 0o111, "Le script doit être exécutable."

def test_setup_dev_env_runs():
    result = subprocess.run([SCRIPT, '--dry-run'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=20)
    assert result.returncode == 0, f"Le script setup_dev_env.sh a échoué: {result.stderr.decode()}"

def test_setup_dev_env_no_secrets_in_output():
    result = subprocess.run([SCRIPT, '--dry-run'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=20)
