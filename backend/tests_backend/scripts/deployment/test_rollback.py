...
def test_rollback_script_permissions():
    st = os.stat(SCRIPT)
    assert stat.S_IMODE(st.st_mode) & 0o111, "Le script doit être exécutable."

def test_rollback_script_runs():
    result = subprocess.run([SCRIPT, '--dry-run'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=20)
    assert result.returncode == 0, f"Le script rollback.sh a échoué: {result.stderr.decode()}"

def test_rollback_script_no_secrets_in_output():
    result = subprocess.run([SCRIPT, '--dry-run'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=20)
