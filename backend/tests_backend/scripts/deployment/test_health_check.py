...
def test_health_check_script_permissions():
    st = os.stat(SCRIPT)
...
def test_health_check_script_runs():
    ...
    assert result.returncode == 0, f"Le script health_check.sh a échoué: {result.stderr.decode()}"

def test_health_check_script_no_secrets_in_output():
    result = subprocess.run([SCRIPT, '--dry-run'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=20)
