...
def test_env_development_exists():
    assert ENV_FILE.exists(), f"{ENV_FILE} does not exist"

def test_env_development_required_vars():
    env = load_env_file(ENV_FILE)
    missing = validate_env_vars(env, REQUIRED_VARS)
    assert not missing, f"Missing variables: {missing}"

def test_env_development_debug_and_log_level():
    env = load_env_file(ENV_FILE)
    assert env["DEBUG"].lower() == "true"
    assert env["LOG_LEVEL"] == "DEBUG"

def test_env_development_postgres_config():
    env = load_env_file(ENV_FILE)
    assert env["POSTGRES_HOST"] == "localhost"
    assert env["POSTGRES_PORT"] == "5432"
    assert env["POSTGRES_DB"].endswith("_dev")
    assert env["POSTGRES_USER"] == "dev_user"
    assert env["POSTGRES_PASSWORD"] == "dev-password"

def test_env_development_spotify_config():
    env = load_env_file(ENV_FILE)
    assert env["SPOTIFY_CLIENT_ID"].startswith("dev-")
    assert env["SPOTIFY_CLIENT_SECRET"].startswith("dev-")
    assert env["SPOTIFY_REDIRECT_URI"].startswith("http://localhost")

def test_env_development_email():
    env = load_env_file(ENV_FILE)
    assert env["EMAIL_HOST"] in ("smtp.mailtrap.io", "localhost")

def test_env_development_no_plain_secrets():
    env = load_env_file(ENV_FILE)
    check_no_weak_secrets(env)

def test_env_development_format():
    env = load_env_file(ENV_FILE)
