...
def test_validate_training_input():
    ...
    result = None
    try:
        from backend.app.tasks.ai_tasks import model_training
        result = getattr(model_training, 'validate_training_input')()
...
def test_train_model_task():
    ...
    try:
        from backend.app.tasks.ai_tasks import model_training
        result = getattr(model_training, 'train_model_task')()
    except Exception as exc:
        pytest.fail('Erreur lors de l\'appel réel : {}'.format(exc))
    assert result is not None
